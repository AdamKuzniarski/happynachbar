...
export function AdminSchell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6">
      <div className="grid gap-4 sm:grid-cols-[240px_1fr] sm:gap-6">
        <aside className="rounded-md border-2 border-fern bg-surface p-3 sm:p-4">
          ...
          <nav className="mt-3 flex gap-2 sm:flex-col">
            {NAV.map((item) => (
              <Link
                key={item.href}
                href={item.href}
